...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // ══════════════════════════════════════════
    //  MONITORING — Alerte CEO si paiement down
    // ══════════════════════════════════════════
    if (body.event === 'health_check') {
      const APP_URL = process.env.APP_URL || 'https://funnelafrica.vercel.app';
      const CEO_EMAIL = 'kessahoud@gmail.com';
      const startOfDay = new Date(); startOfDay.setHours(0,0,0,0);

      // Vérifier les paiements des dernières 24h
      const { data: recentOrders, error: ordersErr } = await sb.from('orders')
        .select('id, payment_status, created_at, amount')
        .gte('created_at', new Date(Date.now() - 24*60*60*1000).toISOString())
        .order('created_at', { ascending: false });

      const total    = (recentOrders||[]).length;
      const paid     = (recentOrders||[]).filter(o=>o.payment_status==='paid').length;
      const pending  = (recentOrders||[]).filter(o=>o.payment_status==='pending').length;
      const failed   = (recentOrders||[]).filter(o=>o.payment_status==='failed').length;
      const revenue  = (recentOrders||[]).filter(o=>o.payment_status==='paid').reduce((s,o)=>s+(o.amount||0),0);

      // Vérifier les paiements bloqués (pending > 1h)
      const stuckOrders = (recentOrders||[]).filter(o => {
        return o.payment_status === 'pending' &&
               new Date(o.created_at) < new Date(Date.now() - 60*60*1000);
      });

      const status = { total, paid, pending, failed, stuck: stuckOrders.length, revenue };
      console.log('[HEALTH CHECK]', JSON.stringify(status));

      // Alerte si problème détecté
      const hasIssue = failed > 3 || stuckOrders.length > 5 || (total > 10 && paid === 0);
      if (hasIssue) {
        await fetch(`${APP_URL}/api/email/sequence`, {
          ...
          body: JSON.stringify({
            ...
            to_email: CEO_EMAIL,
            to_name: 'Samuel CEO',
            _override_subject: '🚨 ALERTE FunnelAfrica — Problème paiements détecté',
            _override_body: `Rapport de monitoring FunnelAfrica (dernières 24h):

Commandes total: ${total}
Payées: ${paid}
En attente: ${pending}
Échouées: ${failed}
Bloquées (>1h): ${stuckOrders.length}
Revenu: ${revenue.toLocaleString('fr')} FCFA

ACTION REQUISE: Vérifier FedaPay et les logs Vercel.`
          })
        }).catch(e => console.error('Alert email:', e));
      }

      return res.status(200).json({ ok: true, status, alert_sent: hasIssue });
    }

    // ══════════════════════════════════════════
    //  WHATSAPP AUTOMATION — Vendeur inactif
    // ══════════════════════════════════════════
    if (body.event === 'whatsapp_inactive' || (isGet && false)) {
      // Vendeurs sans vente depuis 7 jours → message WhatsApp + email
      const { data: inactive } = await sb.from('profiles')
        .select('id, full_name, email, created_at, payout_balance')
        .lt('created_at', new Date(Date.now() - 7*24*60*60*1000).toISOString());

      let notified = 0;
      for (const vendor of (inactive||[])) {
        // Vérifier si a des ventes récentes
        const { data: recentSales } = await sb.from('orders')
          .select('id').eq('user_id', vendor.id).eq('payment_status', 'paid')
          .gte('created_at', new Date(Date.now() - 7*24*60*60*1000).toISOString())
          .limit(1);

        if (recentSales && recentSales.length > 0) continue; // A vendu → skip

        // Envoyer email de réactivation
        await seq('relance', {
          to_email: vendor.email,
          to_name:  vendor.full_name || vendor.email.split('@')[0]
        });
        notified++;
        await new Promise(r => setTimeout(r, 200)); // Anti-spam
      }
      return res.status(200).json({ success: true, event: 'whatsapp_inactive', notified });
    }

    return res.status(400).json({ error: 'Événement non reconnu. Utilisez : cron | signup | manual | health_check | whatsapp_inactive' });
