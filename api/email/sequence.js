  ...
  try {
    ...
    if (type === 'vendor_broadcast') {
      ...
      for (const contact of contacts) {
        const { wrap, p, btn, divider } = require('../../lib/email-templates');
    ...
    if (type === 'abandon_cart') {
      if (!buyer_email || !buyer_name) return res.status(400).json({ error: 'buyer_email et buyer_name requis' });
      const { tplAbandonCart } = require('../../lib/email-templates');
    ...
    if (type === 'refund') {
      if (!buyer_email || !buyer_name) return res.status(400).json({ error: 'buyer_email et buyer_name requis' });
      const { tplRefund } = require('../../lib/email-templates');
      const tpl = tplRefund({
        buyer_name, product_name: req.body.product_name,
    ...
    if (type === 'upsell_offer') {
      if (!buyer_email || !buyer_name) return res.status(400).json({ error: 'buyer_email et buyer_name requis' });
      const { tplUpsellOffer, generateAITip } = require('../../lib/email-templates');
    ...
    if (type === 'affiliate_welcome') {
      ...
      const { tplAffiliateWelcome } = require('../../lib/email-templates');
    ...
    if (type === 'weekly_digest') {
      ...
      const { tplWeeklyDigest, generateAITip } = require('../../lib/email-templates');
    ...
    if (type === 'review_request') {
      ...
      const { wrap, h1, p, btn, divider } = require('../../lib/email-templates');
      const html = wrap(`
